 ...
 */
    ...
    int server_type;                       ///< 0 for main and 1 for backup, default is 0
...
server_type和group_id用于主备功能。所有upstream必需有type为MAIN的server，否则upstream不可用。  
类型为BACKUP的server，会在同group_id的MAIN熔断情况下被使用。
