...
{
	KafkaBlock block;
	int message_size = 4 + 1 + 1 + 4 + 4 +
		record->get_key_len() + record->get_value_len();
	...
	append_bytes(&cur, (const char *)record->get_key(), record->get_key_len());
	append_nullable_bytes(&cur, (const char *)record->get_value(), record->get_value_len());
								...
								int64_t first_timestamp, void *env)
{
	...
	if (record->get_key_len() > 0)
		append_varint_i32(klen_str, (int32_t)record->get_key_len());
	else
		append_varint_i32(klen_str, (int32_t)-1);

	if (record->get_value())
		append_varint_i32(vlen_str, (int32_t)record->get_value_len());
	else
	...
	int length = 1 + timestamp_delta_str.size() + offset_delta_str.size() +
		klen_str.size() + record->get_key_len() + vlen_str.size() +
		record->get_value_len() + hdr_cnt_str.size() + hdr_str.size();
	...
	if (record->get_key_len() > 0)
		append_string_raw(&cur, (const char *)record->get_key(), record->get_key_len());

	append_string_raw(&cur, vlen_str);
	if (record->get_value_len() > 0)
		append_string_raw(&cur, (const char *)record->get_value(), record->get_value_len());
...
int KafkaMessage::encode_head()
{
	...
	append_i32(this->headbuf, 0);
	append_nullable_string(this->headbuf, "workflow", 8);
