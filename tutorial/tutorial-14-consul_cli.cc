...
void print_discover_result(std::vector<struct protocol::ConsulServiceInstance>& discover_result)
{
	for (const auto& instance : discover_result)
	{
		...
		fprintf(stderr, "service_address:%s\n", instance.service.service_address.address.c_str());
		fprintf(stderr, "service_port:%d\n", instance.service.service_address.port);
		...
		fprintf(stderr, "lan:%s:%d\n", instance.service.lan.address.c_str(),
									   instance.service.lan.port);
		fprintf(stderr, "lan_ipv4:%s:%d\n",
									instance.service.lan_ipv4.address.c_str(),
									instance.service.lan_ipv4.port);
		fprintf(stderr, "lan_ipv6:%s:%d\n",
									instance.service.lan_ipv6.address.c_str(),
									instance.service.lan_ipv6.port);
		fprintf(stderr, "wan:%s:%d\n", instance.service.wan.address.c_str(),
									   instance.service.wan.port);
		fprintf(stderr, "wan_ipv4:%s:%d\n",
									instance.service.wan_ipv4.address.c_str(),
									instance.service.wan_ipv4.port);
		fprintf(stderr, "wan_ipv6:%s:%d\n",
									instance.service.wan_ipv6.address.c_str(),
									instance.service.wan_ipv6.port);
...
void consul_callback(WFConsulTask *task)
{
	...
	{
		fprintf(stderr, "error:%d, http_error:%s, error msg:%s\n",
				 error, task->get_error_reason().c_str(),
...
int main(int argc, char *argv[])
{
	...
	config.set_use_health_check(true);
	...
	else if (0 == strcmp(argv[2], "register"))
	{
		...
		init_consul_service(&service);
		service.tags.emplace_back("tag1");
		service.tags.emplace_back("tag2");
		service.service_address.address = "127.0.0.1";
		service.service_address.port = 8000;
