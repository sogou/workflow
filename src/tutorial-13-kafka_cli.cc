...
int main(int argc, char *argv[])
{
	...
	else if (argv[2][0] == 'c')
	{
		if (argc > 3 && argv[3][0] == 'd')
		{
            ...
            config.set_offset_timestamp(1634799992000LL);
            config.set_client_id("workflow");
            task->set_config(std::move(config));

			KafkaToppar toppar;
			toppar.set_topic_partition("workflow_test1", 1);
			//toppar.set_offset(0);
			//toppar.set_offset_timestamp(1634697370000LL);
			task->add_toppar(toppar);

			toppar.set_topic_partition("workflow_test2", 1);
			//toppar.set_offset(1);
			task->add_toppar(toppar);
